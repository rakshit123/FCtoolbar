...
ENTRY("AFNI_compfc_CB");
    ...
    int ns = im3d->fim_now->dblk->nvals ; // num of series
    int nv = DSET_NVOX(im3d->fim_now);  // num of voxels
    ...
    int vx = (int)((im3d->vinfo->xi-DSET_XORG(im3d->fim_now))/DSET_DX(im3d->fim_now));
    int vy = (int)((im3d->vinfo->yj-DSET_YORG(im3d->fim_now))/DSET_DY(im3d->fim_now));
    int vz = (int)((im3d->vinfo->zk-DSET_ZORG(im3d->fim_now))/DSET_DZ(im3d->fim_now));
    int index = DSET_ixyz_to_index(im3d->fim_now,vx,vy,vz);
    ...
    for(k=0;k<nv;k++){
      ...
      for(j=0;j<ns;j++){
      //  if(typ == MRI_float){
          ts = (short *) DSET_ARRAY(im3d->fim_now,j) ;
      //  }else{
      ...
      corr[k] = (ns*exy-ex*ey)/(sqrt(ns*exx-ex*ex)*sqrt(ns*eyy-ey*ey));
    }
    ...
    float * mb1 = (float * ) DSET_ARRAY(im3d->fim_now,0);
    for(j=0;j<nv;j++){
      printf("%f %f %s", (float)mb[j], (float)corr[j], "\n");
    } 
