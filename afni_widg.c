...
***/
   ...
   if( im3d->type == AFNI_3DDATA_VIEW ){
      imag->seedfc=
         XtVaCreateManagedWidget(
            "dialog" , xmPushButtonWidgetClass , imag->popmenu ,
               LABEL_ARG("Compute Functional Connectivity") ,
               XmNmarginHeight , 0 ,
               XmNtraversalOn , True  ,
               XmNinitialResourcesPersistent , False ,
            NULL ) ;
      XtAddCallback( imag->seedfc , XmNactivateCallback ,
                     AFNI_imag_pop_CB , im3d ) ;
   } else {
      imag->seedfc = NULL ;
   }
   ...
   if( im3d->type == AFNI_3DDATA_VIEW ){
      imag->fctoolbar=
         XtVaCreateManagedWidget(
            "dialog" , xmPushButtonWidgetClass , imag->popmenu ,
               LABEL_ARG("Setup Functional Connectivity") ,
               XmNmarginHeight , 0 ,
               XmNtraversalOn , True  ,
               XmNinitialResourcesPersistent , False ,
            NULL ) ;
      XtAddCallback( imag->fctoolbar , XmNactivateCallback ,
                     AFNI_imag_pop_CB , im3d ) ;
   } else {
      imag->fctoolbar = NULL ;
   }
